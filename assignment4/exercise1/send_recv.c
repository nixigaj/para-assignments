...
int main(int argc, char **argv) {
    ...
    if (mpi_rank == ROOT_RANK) {
        ...
        for (int i = 1; i < mpi_size; i++) {
            MPI_Send(&cfg.max_number, 1, MPI_UINT64_T, i, 0, MPI_COMM_WORLD);
        }
    } else {
        MPI_Recv(&cfg.max_number, 1, MPI_UINT64_T, ROOT_RANK, 0, MPI_COMM_WORLD, &mpi_status);
    }
    ...
    if (mpi_rank == ROOT_RANK) {
        ...
        // Receive counts from all worker processes
        for (int i = 1; i < mpi_size; i++) {
            uint64_t worker_count = 0;
            MPI_Recv(&worker_count, 1, MPI_UINT64_T, i, 0, MPI_COMM_WORLD, &mpi_status);
            total_primes += worker_count;
        }
    } else {
        // Send my local count to the root (This will be 0 in the edge case, or the chunk count otherwise)
        MPI_Send(&local_count, 1, MPI_UINT64_T, ROOT_RANK, 0, MPI_COMM_WORLD);
    }
