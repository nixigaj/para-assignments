...
typedef struct worker_context {
    pthread_t thread;
    bool *is_prime; // Shared array between threads
    uint64_t max_number;
...
static void *worker(void *in_data) {
    ...
    const uint64_t seq_max = (uint64_t)sqrt((double)ctx->max_number);

    // Iterate through seed primes
    for (uint64_t p = 2; p <= seq_max; ++p) {
        if (ctx->is_prime[p]) {

            // Find the first multiple of p within the chunk of this thread
            uint64_t start = ctx->chunk_start / p * p;

            // If the initial computed start happens lands below start
            if (start < ctx->chunk_start) {
                start += p;
            }

            // It is important that finding the first multiple happens in
            // constant time, which is done above.

            // Mark all multiples of p in the chunk
            for (uint64_t i = start; i <= ctx->chunk_end; i += p) {
                ctx->is_prime[i] = false;
            }
        }
    }
    pthread_exit(NULL);
...
static void distribute_chunks(worker_context_t *workers, const uint32_t num_workers, const uint64_t max_number, bool *sieve_array) {
    const uint64_t parallel_start = (uint64_t)sqrt((double)max_number) + 1;

    // Edge case where no parallel work is needed
    if (parallel_start > max_number) {
        for (uint32_t i = 0; i < num_workers; i++) {
            workers[i].is_prime = sieve_array;
            workers[i].max_number = max_number;
            workers[i].chunk_start = 0;
            workers[i].chunk_end = 0;
        }
        return;
    }

    const uint64_t range_size = max_number - parallel_start + 1;
    const uint64_t base_chunk = range_size / num_workers;
    const uint64_t remainder = range_size % num_workers;
    uint64_t current_start = parallel_start;

    for (uint32_t i = 0; i < num_workers; i++) {
        uint64_t chunk_size = base_chunk;
        if (i < remainder) {
            chunk_size += 1;
        }
        workers[i].is_prime = sieve_array;
        workers[i].max_number = max_number;
        workers[i].chunk_start = current_start;
        workers[i].chunk_end = current_start + chunk_size - 1;
...
int main(const int argc, char **argv) {
    ...
    distribute_chunks(workers, cfg.num_threads, cfg.max_number, is_prime);
    ...
    for (uint32_t i = 0; i < cfg.num_threads; ++i) {
        ...
        if (ret != 0) {
            ...
            free(is_prime); free(workers);
            exit(EXIT_FAILURE);
        }
    }

    for (uint32_t i = 0; i < cfg.num_threads; ++i) {
        // Once again, skip threads with no work
        if (workers[i].chunk_end == 0) {
            continue;
        }
